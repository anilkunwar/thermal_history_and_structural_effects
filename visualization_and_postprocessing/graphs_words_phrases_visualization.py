...
publication-quality word clouds, bibliometric networks, and radar charts with customizable colormaps and styles. Adjust visual aspects
like label sizes, line thicknesses, and transparency using sliders. The app extracts text using PyPDF2, processes keywords and phrases
with NLTK and spaCy, and visualizes using WordCloud, NetworkX, and Matplotlib.
""")
...
def generate_bibliometric_network(text, selected_keywords, tfidf_scores, label_font_size, selection_criteria, node_colormap, edge_colormap, network_style, line_thickness, node_alpha, edge_alpha, title_font_size, caption_font_size):
    try:
        stop_words = set(stopwords.words('english'))
        stop_words.update(['laser', 'microstructure', 'the', 'a', 'an', 'preprint', 'submitted', 'manuscript'])
        stop_words.update([w.strip().lower() for w in custom_stopwords_input.split(",") if w.strip()])

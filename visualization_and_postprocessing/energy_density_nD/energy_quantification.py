...
<style>
    ...
    .stButton > button {
        background-color: #6366f1; color: white; border-radius: 10px; font-weight: 600;
        border: none; padding: 0.6rem 1.2rem; transition: all 0.2s ease;
    }
    .stButton > button:hover {background-color: #4f46e5; transform: translateY(-1px); box-shadow: 0 6px 12px rgba(99,102,241,0.25);}
    .header {
        font-size: 2.8em; color: #1e293b; text-align: center; margin-bottom: 8px;
        font-weight: 800; letter-spacing: -0.5px;
    }
    .subtitle {
        text-align: center; color: #64748b; font-size: 1.15em;
        max-width: 800px; margin: 0 auto 30px auto; line-height: 1.5;
    }
    .section-title {
        font-size: 1.35em; color: #334155; font-weight: 700;
        margin: 25px 0 15px 0; display: flex; align-items: center; gap: 8px;
    }
    .param-label {
        font-weight: 600; color: #475569; font-size: 0.95em; margin-bottom: 4px;
    }
    .formula-box {
    ...
    .footer {
        ...
        margin-top: 50px; padding-top: 25px; border-top: 1px solid #e2e8f0;
    }
</style>
...
# ============================================================
# SCHEMATIC (100% go.Scatter — robust across Plotly versions)
# ============================================================
def create_schematic(P, v, h_um, t_um):
    ...
    fig.add_annotation(x=10.1, y=(y1 + y2) / 2, text=f"<b>l<sub>h</sub></b>={h_um:.0f} μm",
                       showarrow=False, font=dict(color="#059669", size=11), row=1, col=1)

    theta = np.linspace(0, 2 * np.pi, 60)
    cx, cy, r = 5.0, y_tracks[2], 0.55
    fig.add_trace(go.Scatter(
        x=cx + r * np.cos(theta), y=cy + r * np.sin(theta),
        mode="lines", fill="toself", fillcolor="rgba(239,68,68,0.20)",
        line=dict(color="#dc2626", width=2),
        name="Laser Spot", hoverinfo="skip"
    ), row=1, col=1)
    fig.add_annotation(x=cx, y=cy, text=f"<b>{P:.0f} W</b>",
                       showarrow=False, font=dict(size=10, color="#991b1b"), row=1, col=1)
    fig.add_annotation(x=5, y=7.5, text=f"<b>v<sub>scan</sub></b> = {v:.0f} mm/s",
                       showarrow=False, font=dict(color="#4338ca", size=12), row=1, col=1)

    # ---------- SIDE VIEW ----------
    t_vis = max(0.6, t_mm * 25)
    ...
    fig.add_annotation(x=5, y=1.5, text="Substrate / Prior Layers",
                       showarrow=False, font=dict(color="white", size=12), row=1, col=2)
    ...
    fig.add_annotation(x=5, y=3 + t_vis / 2, text="Current Layer",
                       showarrow=False, font=dict(color="#92400e", size=12), row=1, col=2)
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines", fill="toself", fillcolor="rgba(239,68,68,0.35)",
        line=dict(color="#dc2626", width=2),
    ...
    fig.add_annotation(x=5, y=3 + t_vis + beam_h + 0.4,
                       ...
                       showarrow=False, font=dict(color="#991b1b", size=12), row=1, col=2)
...
# ============================================================
# 1️⃣ SCHEMATIC HERO (TOP CENTERPIECE)
# ============================================================
with st.container(border=True):
    st.markdown('<div class="section-title">📐 Process Schematic</div>', unsafe_allow_html=True)
    st.caption("Interactive geometry showing scan strategy, hatch spacing, and layer build-up.")
    st.plotly_chart(create_schematic(
        st.session_state.laser_power_slider,
        st.session_state.scan_speed_slider,
        st.session_state.hatch_spacing_slider,
        st.session_state.layer_thickness_slider
    ), use_container_width=True)

# ============================================================
# 2️⃣ PARAMETERS & RESULTS
# ============================================================
...
with col_inputs:
    ...
    st.markdown('<p style="color:#64748b; font-size:0.9em; margin-bottom:15px;">Number inputs and sliders are synchronized. Edit either one.</p>', unsafe_allow_html=True)
...
with col_results:
    st.markdown('<div class="section-title">📈 Energy Density Results</div>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        st.caption("Energy per unit length along the scan direction")
    ...
    with st.container(border=True):
        ...
        st.caption("Energy per unit area of the scanned surface")
    ...
    with st.container(border=True):
        ...
        st.caption("Energy per unit volume of the deposited layer")
...
with st.expander("📋 Parameter Reference & Typical LPBF Ranges"):
    ...
    with c1:
        st.metric("🔦 Laser Power", f"{P:.1f} W",
                  help="Optical power delivered by the laser source.")
    with c2:
        st.metric("⚡ Scan Speed", f"{v:.1f} mm/s",
                  help="Velocity of laser beam movement.")
    with c3:
        st.metric("↔️ Hatch Spacing", f"{h_um:.1f} μm",
                  help="Distance between adjacent scan tracks.")
    with c4:
        st.metric("📏 Layer Thickness", f"{t_um:.1f} μm",
                  help="Powder layer thickness per build cycle.")

# ============================================================
# VISUALIZATIONS
# ============================================================
st.markdown('<div class="section-title">📊 Energy Density Visualization</div>', unsafe_allow_html=True)

tab1, tab2 = st.tabs(["📉 Trends vs. Scan Speed", "🗺️ 2D Process Window (VED)"])

with tab1:
    st.markdown("Compare how each metric evolves with scanning speed. Your current speed is marked.")
    ...
    fig_trends = make_subplots(
        ...
        subplot_titles=("Linear Energy Density  (J/mm)", "Areal Energy Density  (J/mm²)", "Volumetric Energy Density  (J/mm³)")
    )
    ...
    for p, color in zip(powers, colors):
        ...
        fig_trends.add_trace(go.Scatter(
            ...
            line=dict(color=color, width=2.5), legendgroup=str(p), showlegend=True,
            hovertemplate="%{x:.0f} mm/s<br>%{y:.4f} J/mm<<extra></extra>"
        ), row=1, col=1)
        fig_trends.add_trace(go.Scatter(
            x=scan_speeds, y=aed_vals, mode="lines", name=f"{p} W",
            line=dict(color=color, width=2.5, dash="dash"), legendgroup=str(p), showlegend=False,
            hovertemplate="%{x:.0f} mm/s<br>%{y:.4f} J/mm²<<extra></extra>"
        ), row=2, col=1)
        fig_trends.add_trace(go.Scatter(
            x=scan_speeds, y=ved_vals, mode="lines", name=f"{p} W",
            line=dict(color=color, width=2.5, dash="dot"), legendgroup=str(p), showlegend=False,
            hovertemplate="%{x:.0f} mm/s<br>%{y:.4f} J/mm³<<extra></extra>"
        ), row=3, col=1)
...
with tab2:
    st.markdown("Interactive VED process map. The white **×** marks your current settings.")
...
# INSIGHTS
# ============================================================
st.markdown('<div class="section-title">💡 Process Optimization Insights</div>', unsafe_allow_html=True)
...
with c2:
    ...
    **Current:** `{aed:.4f} J/mm²`
    ...
    • Accounts for hatch spacing  
    • Optimal AlSiMg: **60 – 140 J/mm²**  
    • Controls overlap & finish  
    • Minimizes balling defects
    """)
...
with c3:
    ...
    **Current:** `{ved:.4f} J/mm³`
    ...
    • Most predictive for bulk properties  
    • Target >99.5% density: **55 – 95 J/mm³**  
    • Balance: porosity vs. grain growth  
...
with c2:
    export_data = {
        "timestamp": "2025",
        "parameters": {"laser_power_W": P, "scan_speed_mm_s": v, "hatch_spacing_um": h_um, "layer_thickness_um": t_um},
        "results": {"LED_J_per_mm": round(led, 6), "AED_J_per_mm2": round(aed, 6), "VED_J_per_mm3": round(ved, 6)},

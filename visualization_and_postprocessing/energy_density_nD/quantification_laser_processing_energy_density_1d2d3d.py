...
# KaTeX for high-quality LaTeX rendering
st.markdown("""
    ...
    for param, config in params.items():
        ...
        with col_sld:
            slide_val = st.slider(
                ...
                step=config['step'],
                key=f"{param}_sld",
                label_visibility="collapsed"  # Keep collapsed but suppress warning by providing aria-label via help
            )
        
        # Sync: use whichever was most recently changed
...
# ═══════════════════════════════════════════════════════════════
# SCHEMATIC REPRESENTATION: Rectangular Slab Geometry (FIXED)
# ═══════════════════════════════════════════════════════════════
...
def create_schematic(P, v, h_um, t_um):
    """Create interactive Plotly schematic showing laser processing geometry - FIXED for 3D annotations"""
    h_mm = h_um / 1000
    t_mm = t_um / 1000
    ...
    # Top face (offset by exaggerated layer thickness for visibility)
    z_top_val = t_mm * 15
    z_top = [z_top_val] * 5
    fig.add_trace(go.Scatter3d(
    ...
    scan_y = 3
    fig.add_trace(go.Scatter3d(
        x=[1, 9], y=[scan_y, scan_y], z=[z_top_val+0.3, z_top_val+0.3],
    ...
    # Scan direction arrow (as a separate trace)
    fig.add_trace(go.Scatter3d(
        x=[9, 9.8], y=[scan_y, scan_y], z=[z_top_val+0.3, z_top_val+0.3],
    ...
    fig.add_trace(go.Scatter3d(
        x=[1, 9], y=[scan_y + h_mm*8, scan_y + h_mm*8], z=[z_top_val+0.3, z_top_val+0.3],
        mode='lines', name='Adjacent Scan Track',
        line=dict(color='#9b59b6', width=2, dash='dash'),
        showlegend=True
    ), row=1, col=1)
    
    # ✅ FIXED: Use Scatter3d with mode='text' for 3D labels instead of add_annotation with z
    # Laser Power label
    fig.add_trace(go.Scatter3d(
        x=[5], y=[scan_y], z=[z_top_val+1.5],
        mode='text',
        text=[f'<b>P<sub>l</sub></b><br>{P} W'],
        textposition='top center',
        textfont=dict(size=11, color='#e74c3c', family='Arial'),
        name='Laser Power Label',
        hoverinfo='skip',
        showlegend=False
    ), row=1, col=1)
    ...
    # Scan Speed label
    fig.add_trace(go.Scatter3d(
        x=[9.5], y=[scan_y+0.8], z=[z_top_val+0.3],
        mode='text',
        text=[f'<b>v<sub>scan</sub></b><br>{v} mm/s'],
        textposition='middle right',
        textfont=dict(size=11, color='#2ecc71', family='Arial'),
        name='Scan Speed Label',
        hoverinfo='skip',
        showlegend=False
    ), row=1, col=1)
    
    # Hatch spacing label (3D)
    fig.add_trace(go.Scatter3d(
        x=[1], y=[scan_y + h_mm*4], z=[z_top_val+0.5],
        mode='text',
        text=[f'<b>l<sub>h</sub></b><br>{h_um} μm'],
        textposition='middle left',
        textfont=dict(size=10, color='#9b59b6', family='Arial'),
        name='Hatch Label',
        hoverinfo='skip',
        showlegend=False
    ), row=1, col=1)
    ...
    # Hatch spacing label (2D annotation - valid)
    fig.add_annotation(
        x=10.1, y=(y1+y2)/2,
        ...
        xref='x2', yref='y2',  # Reference the second subplot's axes
        row=1, col=2
    )
    
    # Layer thickness indicator
    fig.add_annotation(
        x=5, y=-0.8,

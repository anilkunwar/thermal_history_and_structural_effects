...
Enter a term or phrase, select a corpus size estimation preference, and the app will query arXiv to estimate
TF (based on term occurrences in abstracts), IDF (based on document frequency), and TF-IDF in materials
science and applied physics categories. The actual number of documents scanned (N) is dynamically estimated
based on your preference: Speed (fast, less accurate), Optimization (balanced), Accuracy (slower, more accurate),
or Offline (precomputed, instant). The result is displayed and saved as a JSON file.
""")

# Function to estimate corpus size (N)
def estimate_corpus_size(preference):
    try:
        # Define max_results based on preference
        if preference == "Speed":
            max_results = 100
            scale_factor = 1000  # Larger scale due to smaller sample
        elif preference == "Accuracy":
            max_results = 5000
            scale_factor = 20  # Smaller scale due to larger sample
        elif preference == "Optimization":
            max_results = 1000
            scale_factor = 100  # Balanced scale
        else:  # Offline
            return 100000  # Precomputed fallback
        ...
        sample_count = sum(1 for _ in search.results())
        N = sample_count * scale_factor  # Scale up sample to estimate total
        return max(N, 1)  # Avoid division by zero
    except Exception as e:
        logger.error(f"Error estimating corpus size: {str(e)}")
        return 100000  # Fallback to default estimate

# Dropdown for corpus size estimation preference
estimation_preference = st.selectbox(
    "Select Corpus Size Estimation Preference",
    options=["Speed", "Optimization", "Accuracy", "Offline"],
    index=1,  # Default to Optimization
    help="Speed: Fast (~1-2s, less accurate). Optimization: Balanced (~2-4s). Accuracy: Slower (~5-15s, more accurate). Offline: Instant (uses precomputed value)."
)

# Estimate corpus size (cached to avoid repeated queries)
if 'corpus_size' not in st.session_state or st.button("Re-estimate Corpus Size"):
    if estimation_preference != "Offline":
        with st.spinner(f"Estimating corpus size with {estimation_preference.lower()} preference..."):
            st.session_state.corpus_size = estimate_corpus_size(estimation_preference)
            time.sleep(1)  # Respect rate limits
    else:
        st.session_state.corpus_size = 100000  # Offline fallback
N = st.session_state.corpus_size
st.write(f"**Estimated corpus size (N)**: {N} documents (materials science and applied physics, using {estimation_preference} preference)")
    ...
    if not term.strip():
        ...
        with st.spinner(f"Querying arXiv for '{term}'..."):
            try:
                ...
                tf_idf_dict = {term: {"TF": tf, "IDF": idf, "TF-IDF": tf_idf, "Corpus Size (N)": N, "Estimation Preference": estimation_preference}}
                try:
...
2. Use the TF, IDF, TF-IDF, Corpus Size (N), and Estimation Preference values in your application. Example:
   ```python
   import json
   with open("tf_idf_single_term.json", "r") as f:
       tf_idf_dict = json.load(f)
   term = "selective laser melting"
   metrics = tf_idf_dict.get(term, {"TF": 0.0, "IDF": math.log(100000 / 10000), "TF-IDF": 0.0, "Corpus Size (N)": 100000, "Estimation Preference": "Offline"})
   print(f"Corpus Size (N): {metrics['Corpus Size (N)']} (Preference: {metrics['Estimation Preference']})")

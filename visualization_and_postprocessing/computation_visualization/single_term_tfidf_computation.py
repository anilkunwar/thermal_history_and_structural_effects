    ...
    if not term.strip():
        ...
        with st.spinner(f"Querying arXiv for '{term}'..."):
            try:
                ...
                tf_idf_dict = {term: {"TF": tf, "IDF": idf, "TF-IDF": tf_idf}}
                try:
...
2. Use the TF, IDF, and TF-IDF values in your application. Example:
   ```python
   import json
   with open("tf_idf_single_term.json", "r") as f:
       tf_idf_dict = json.load(f)
   term = "selective laser melting"
   metrics = tf_idf_dict.get(term, {"TF": 0.0, "IDF": math.log(100000 / 10000), "TF-IDF": 0.0})

...
publication-quality word clouds, bibliometric networks, and radar charts with customizable colormaps. The app extracts text using PyPDF2,
processes keywords and phrases with NLTK and spaCy, and visualizes using WordCloud, NetworkX, and Matplotlib.
""")
...
def generate_word_cloud(text, selected_keywords, tfidf_scores, selection_criteria, colormap):
    try:
        ...
        plt.figtext(
            0.5, 0.05, caption, ha="center", fontsize=10, wrap=True,
...
def generate_bibliometric_network(text, selected_keywords, tfidf_scores, label_font_size, selection_criteria, node_colormap, edge_colormap):
    try:
        ...
        plt.figtext(
            0.5, 0.06, caption, ha="center", fontsize=10, wrap=True,
            bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.5')
        )
...
def generate_radar_chart(selected_keywords, values, title, selection_criteria, colormap, max_keywords):
    try:
        ...
        ax.fill(angles, vals, color=fill_color, alpha=0.3)
    ...
    with st.spinner("Processing PDF..."):
        ...
        if "Error" in text:
            ...
            if "Error" in selected_text or "not found" in selected_text:
                ...
                criteria_parts.append(f"frequency radar colormap: {freq_radar_colormap}")
                criteria_parts.append(f"tfidf radar colormap: {tfidf_radar_colormap}")

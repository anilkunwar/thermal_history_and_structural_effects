...
# Parameter validation ranges (expanded for negative SFE and high temperatures)
valid_ranges = {
...
# PMI calculation for phrase detection
...
# Extract parameters using regex and SpaCy
...
# Visualize results with enhanced customization
def visualize_results(df, entity_types, pmi_results):
    ...
    axes_line_color = st.sidebar.color_picker("Axes Line Color", "#808080")  # Default gray
    marker_size = st.sidebar.slider("Scatter Marker Size", 20, 100, 50)
    ...
    colormap = st.sidebar.selectbox("Colormap", colormaps, index=colormaps.index('tab10'))
    ...
    legend_position = st.sidebar.selectbox("Legend Position", legend_positions, index=0)
    
    # Additional customization for histogram and Pearson box
    hist_edge_width = st.sidebar.slider("Histogram Edge Line Width", 0.5, 2.0, 1.0, step=0.1)
    pearson_box_edge_color = st.sidebar.color_picker("Pearson Box Edge Color", "#000000")
    pearson_box_edge_width = st.sidebar.slider("Pearson Box Edge Width", 0.5, 2.0, 0.8, step=0.1)
    ...
    # Update colors based on selected colormap
    cmap = plt.get_cmap(colormap)
    param_colors = {param: cmap(i / len(param_types)) for i, param in enumerate(param_types)}
    if colormap == "tab10":
        param_colors = default_colors  # Revert to default for tab10
    ...
    plt.rcParams.update({
        ...
        'text.usetex': True  # Enable LaTeX for labels
    })
            ...
            if not param_df.empty:
                ...
                if not values.empty:
                    ...
                    counts, bins, _ = ax.hist(values, bins=30 if param_type == "STACKING_FAULT_ENERGY" else 20, 
                                             ...
                                             label=st.sidebar.text_input(f"Legend for {param_type}", param_type.replace('_', ' ').title()))
                    ...
                    # Save and offer histogram counts as CSV
    ...
    if not sfe_df.empty and not temp_df.empty:
        st.subheader("SFE vs Temperature")
        scatter_data = []
            ...
            if not sfe_values.empty and not temp_values.empty:
                for temp, unit in zip(temp_values, temp_units):
                    ...
                    for sfe in sfe_values:
                        scatter_data.append({"Temperature (°C)": temp, "SFE (mJ/m²)": sfe})
        if scatter_data:
            ...
            for spine in ax.spines.values():
                spine.set_color(axes_line_color)
            # Scatter plot
            scatter = ax.scatter(scatter_df["Temperature (°C)"], scatter_df["SFE (mJ/m²)"], 
                                c=param_colors["STACKING_FAULT_ENERGY"], alpha=alpha, s=marker_size,
                                label=st.sidebar.text_input("Legend for SFE vs Temperature", "SFE vs Temperature"))
            ...
            ax.set_title("SFE vs Temperature", fontweight='bold', pad=15, color=title_color)
            ax.grid(True, alpha=0.3)
            
            # Calculate and display Pearson correlation below caption in legend
            legend_label = scatter.get_label()
            if len(scatter_df) >= 2:
                corr, p_value = pearsonr(scatter_df["Temperature (°C)"], scatter_df["SFE (mJ/m²)"])
                legend_label += f"\nPearson r = {corr:.3f}\nP-value = {p_value:.3f}"
            
            # Create legend with single entry combining caption and Pearson stats
            ax.legend([legend_label], loc=legend_position, 
                      bbox_to_anchor=None, 
                      edgecolor=pearson_box_edge_color, 
                      framealpha=0.8, 
                      fontsize=font_size - 2,
                      handlelength=2,  # Show marker for scatter plot
                      handletextpad=0.4)
            
            # Set legend box edge width
            ax.get_legend().get_frame().set_linewidth(pearson_box_edge_width)
            
            plt.tight_layout()
            st.pyplot(fig)
            
            # Save and offer scatter data as CSV
            st.download_button(
                label="Download SFE vs Temperature Scatter CSV",
                data=scatter_df.to_csv(index=False),
                file_name="sfe_vs_temp_scatter.csv",
                mime="text/csv"
            )
            plt.close(fig)
    ...
    if analyze_button:
        if source_type == "Full Text (sfe_knowledgeuniverse.db)":
            ...
            try:
                ...
                pmi_results = {}  # No PMI for uploaded results
...
with tab2:
    ...
    st.markdown("Load previously saved NER results from `.h5`, `.pkl`, or `.pt` files and visualize them with customizable, publication-quality Matplotlib plots. Adjust font size, axes thickness, axes line color, colormap, legend caption, legend position, Pearson box style, histogram edges, and label colors.")
...
4. Run with: `streamlit run sfe_ner_analysis.py`.
